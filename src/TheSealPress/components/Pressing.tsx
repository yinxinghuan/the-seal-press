// Kiln-fire placeholder while gen-image runs. Wet-clay lump glowing
// from within with the AlterU emblem breathing at the centre. Icon /
// silhouette / material stay HIDDEN until reveal — preserve surprise.
interface Props {
  inscription: string;        // possibly empty (blind press)
  blindPress: boolean;
}

export default function Pressing({ inscription, blindPress }: Props) {
  return (
    <div className="tsp-pressing">
      ...
      {inscription && !blindPress && (
        <div className="tsp-pressing__inscription">{inscription}</div>
      )}
      {blindPress && (
        <div className="tsp-pressing__inscription tsp-pressing__inscription--blind">
          — the universe is reading your name —
        </div>
      )}

      <div className="tsp-pressing__foot">
        <em>pressing your seal</em>
        <div className="tsp-loader"><span /><span /><span /></div>
        <div className="tsp-pressing__est">~ 3 min</div>
      </div>
    </div>
  );
}
